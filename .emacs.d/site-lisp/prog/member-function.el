...
(defconst r-match-namespace (rx bol (* blank) (group "namespace" (+? anything)) (* "
") (* blank) "{" (* blank) "
"
))
...
temp buffer when he is done with it."
  (let ((buffer-name (make-temp-name cfile))
    ...
    (save-excursion
      (cond
       ((get-buffer cfile)
        (set-buffer (get-buffer cfile))
        (setq contents (buffer-string)))
       ((file-exists-p cfile)
        (find-file-noselect cfile t)
        (set-buffer (get-buffer cfile))
        (setq contents (buffer-string)))
       )
    ...
    (save-excursion
      (save-restriction
        (let ((temp-file (mf--make-temp-c-file c-file))
              (temp-header (make-temp-name header)))
          (condition-case var
              (progn
                ...
                (goto-char (point-min))
                (let ((inc-header (format "#include \"%s\"\n" header)))
                  (when (and (not (search-forward inc-header nil t))
                ...
                (let ((original-pos nil)
                  ...
                  (setq mf--saved-string
                        (replace-regexp-in-string
                         ...
                         (replace-regexp-in-string
                          ...
                          (replace-regexp-in-string
                           (rx (+ blank) "::" (+ blank)) "::"
                           (mf--format-undefined-decls-from-files temp-header temp-file)))))
                  ...
                  (when (and mf--saved-string
                    ...
                    (delete-trailing-whitespace original-pos (point-max))
                    (goto-char (point-max))
                    (when
                        (<= (skip-chars-backward "\n") -2)
